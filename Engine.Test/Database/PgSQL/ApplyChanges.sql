...
BEGIN
	...
	RETURN QUERY SELECT DISTINCT gateway_ip.id, gateway_ip.address, gateway_ip.port, gateway_ip.protocol, host(gateway_ip.rtp_address), gateway_ip.rtp_port, gateway_ip.rtp_forward, gateway_ip.sip_p_asserted_identity,
	...
	LOOP
		...
		SELECT cache_gateway_price.id, cache_gateway_price.price, cache_gateway_price.currency, cache_gateway_price.price_normalized, cache_gateway_price.timeband
			...
			WHERE number = ANY (p_number)
				...
				AND cache_gateway_price.timeband = 'Flat'::timeband
				AND p_timestamp BETWEEN valid_from AND valid_to
			ORDER BY numberlength DESC LIMIT 1;
		...
		IF NOT FOUND THEN
			...
			SELECT cache_gateway_price.id, cache_gateway_price.price, cache_gateway_price.currency, cache_gateway_price.price_normalized, cache_gateway_price.timeband
				...
				WHERE number = ANY (p_number)
					...
					cache_gateway_price.timeband::timeband IN (
...
CREATE UNIQUE INDEX uq_number_gateway_statistic ON cache_number_gateway_statistics (id);
